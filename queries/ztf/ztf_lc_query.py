...
def split_save(oid_batch,n,i):
	...
	df.to_csv(output_folder+'raw_lcs/lc_{:01d}_{:03d}.csv'.format(n,i), index=False)
...
def obtain_ztf_lightcurves(n):
	...
	for i, oid_batch in enumerate(np.array_split(oids, n_request)):
		if not os.path.isfile(output_folder+'raw_lcs/lc_{:01d}_{:03d}.csv'.format(n,i)):
			...
			try:
				...
				df.to_csv('raw_lcs/lc_{:01d}_{:03d}.csv'.format(n,i), index=False)
...
unable_to_download = []
n_request = 900 #number of requests per core. We require n s.t. len(ztf_oids.txt)/(n_request * n_cores) < 350. 350 is approx the max no. oids we can ask for in http request.
n_cores   = 4
obj = 'qsos'
