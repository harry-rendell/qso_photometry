...
raw_star_data = pd.read_csv(cfg.USER.D_DIR + 'catalogues/calibStars/stripe82calibStars_v4.2.dat', 
                       sep='\s+',
                       comment='#',
                       usecols=[0,1,2,5,13,15,16,19,21,22,25,27,28],
                       names=['uid_s','ra','dec','n_epochs']+[a+b for b in 'gri' for a in ['n_obs_','mag_mean_','mag_mean_err_']],
...
#------------------------------------------------------------------------------
subsample = star_data_filtered.sample(400000, random_state=42, weights=(sum([star_data_filtered['qso_counts_'+b] for b in 'gri'])))
...
for ax, band in zip(axes, 'gri'):
    ...
    ax.hist(grouped_qsos[key], alpha=0.5, label='qsos', **kwargs)
    ax.hist(subsample[key], alpha=0.5, label='star subsample', **kwargs)
...
if SAVE_SUBSAMPLE:
    ...
    subsample.sort_index().to_csv(ddir+'catalogues/calibStars/calibStars_subsample_coords.csv', columns=savecols)
# -
