...
def reader(n_subarray):
	return pd.read_csv(cfg.D_DIR + 'merged/{}/{}_band/lc_{}.csv'.format(obj, band, n_subarray), comment='#', nrows=None, dtype = {'catalogue': np.uint8, 'mag_ps': np.float32, 'magerr': np.float32, 'mjd': np.float64, ID: np.uint32})
...
def savedtdm(uid_dict):
	for key, uids in uid_dict.items():
		...
		df_batch.to_csv(cfg.D_DIR + 'computed/{}/dtdm/raw/{}/dtdm_raw_{}_{}.csv'.format(obj,band,band,key),index=False)
...
# Read uids below
custom = np.loadtxt(cfg.D_DIR + 'computed/{}/dtdm/raw/{}/missing_uids.csv'.format(obj,band), dtype='int', delimiter=',')
chunks = custom
n_cores = len(custom)
