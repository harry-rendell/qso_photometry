...
class singlesurvey_prop():
    ...
    def read_in(self, fname=None, reader=None, multi_proc=True, catalogue_of_properties=None):
        ...
        df = df[~df['oid'].isin(np.loadtxt(cfg.W_DIR + 'analysis/ztf_oids_outside1arcsec.txt',dtype=np.uint64))]
    ...
    def group(self, keys = ['uid'], read_in = True):
        if read_in == True:
            if len(keys) == 1:
                self.df_grouped = pd.read_csv(cfg.D_DIR + 'surveys/ztf/meta_data/ztfdr2_gb_uid_{}.csv'.format(self.band),index_col = 0)
...
class multisurvey_prop():
    ...
    def merge_with_catalogue(self,catalogue='dr12', remove_outliers=True, prop_range_any = {'MBH_MgII':(6,12), 'MBH_CIV':(6,12)}):
        ...
        if catalogue == 'dr12':
            ...
            vac = pd.read_csv(cfg.D_DIR + 'catalogues/SDSS_DR12Q_BH_matched.csv', index_col=16)
    ...
    def group(self, keys = ['uid'], read_in = True):
        if read_in == True:
            if len(keys) == 1:
                self.df_grouped = pd.read_csv(cfg.D_DIR + 'merged/meta_data/df_gb_uid_{}.csv'.format(self.band),index_col = 0)
            elif len(keys) == 2:
                self.df_grouped = pd.read_csv(cfg.D_DIR + 'merged/meta_data/df_gb_uid_cat_{}.csv'.format(self.band),index_col = [0,1])
...
#             self.df_grouped.loc[pd.IndexSlice[:,catalogue],:].index.get_level_values('uid')[(self.df_grouped['mag_count'] > 2)]
    ...
    def calc_dtdm(self, uids=None, n_bins_t = 1000, n_bins_m = 200, t_max=7600, t_spacing = 'log', m_spacing = 'log', read_in = False, key = None):
        ...
        if read_in != False:
            dms_binned = np.loadtxt(cfg.W_DIR + 'analysis/dtdm/dms_binned_{}_{}.csv'.format(key,read_in), delimiter = ',')
    #         dts_binned = np.loadtxt(cfg.W_DIR + 'analysis/dtdm/dts_binned_{}_{}.csv'.format(key,read_in), delimiter = ',')
