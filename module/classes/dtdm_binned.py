...
class dtdm_binned_class():
    ...
    def read(self):
        ...
        with np.load(cfg.D_DIR + f'computed/{self.obj}/dtdm_binned/binned_{self.band}.npz', allow_pickle=True, mmap_mode='r') as data:
    ...
    def hist_dm(self, window_width, overlay_gaussian=False, overlay_lorentzian=False, overlay_exponential=False, overlay_diff=False, cmap=plt.cm.cool, colors=['r','b','g'], alpha=1, save=False):
        ...
        for i, ax in enumerate(axes):
            r2s = []
            m,_,_= ax.hist(self.m_bin_edges[:-1], self.m_bin_edges, weights = self.dms_binned[i], alpha = alpha, density=True, label = self.t_dict[i], color = colors[0]); # *0.25 is for jet color = cmap(0.6+i/20*0.5)
