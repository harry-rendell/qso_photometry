...
class dtdm():
	...
	def read(self):
		...
		Read in binned data
		"""
		self.dts_binned = np.loadtxt(wdir+'data/computed/{}/binned/{}/dt/dts_binned_{}_{}.csv'.format(self.obj, self.subset, self.obj, self.name),  delimiter=',', dtype='uint64')
...
class dtdm_key():
	def __init__(self, obj, name, label, key, n_bins_t, n_bins_m, n_bins_m2, t_max, n_t_chunk, steepness, width, leftmost_bin, verbose=False):
		...
		self.bounds_values = np.loadtxt(wdir+'data/computed/{}/binned/{}/bounds_values.txt'.format(self.obj, self.key))
...
#		 self.stats(verbose)
	...
	def read(self):
		...
		for i in range(n):
			...
			self.dts_binned[i] = np.loadtxt(wdir+'data/computed/{}/binned/{}/dt/dts_binned_{}_{}_{}_{}.csv'.format(self.obj, self.key, self.obj, self.name, self.key, i),  delimiter=',', dtype='uint64')
			self.dms_binned[i] = np.loadtxt(wdir+'data/computed/{}/binned/{}/dm/dms_binned_{}_{}_{}_{}.csv'.format(self.obj, self.key, self.obj, self.name, self.key, i),  delimiter=',', dtype='uint64')
...
class dtdm_raw_analysis():
	...
	def __init__(self, obj, ID, band, name):
		...
		self.ID = ID
		self.band = band
		self.data_path = wdir+'data/computed/{}/dtdm/raw/{}/'.format(obj,band)
		self.name = name
...
#		 self.df.loc[(self.df['cat']<4),'de'] += 5 # increase the error on plate pair observations by 0.05
	...
	def read_key(self, key):
		...
		"""
		self.key = key
		path = wdir+'data/computed/{}/binned/{}/uids/'.format(self.obj, self.key)
...
# 					results['SF b'][i,j,(0,1)] = (subset['dm']**2).mean(), (subset['dm']**2).var()
		...
		if save:
			for key in pooled_results.keys():
				np.savetxt(wdir+'data/computed/{}/dtdm_stats/{}/pooled_{}.csv'.format(self.obj, self.log_or_lin, key.replace(' ','_')), pooled_results[key])
			np.savetxt(wdir+'data/computed/{}/dtdm_stats/{}/mjd_edges.csv'.format(self.obj, self.log_or_lin), self.mjd_edges)
...
# 						SF[mask] = 0
		...
		if save:
			for key in pooled_results.keys():
				for group_idx in range(self.n_groups):
					np.savetxt(wdir+'data/computed/{}/dtdm_stats/{}/{}/pooled_{}_{}.csv'.format(self.obj, prop, self.log_or_lin, key.replace(' ','_'), group_idx), pooled_results[key][:, group_idx])
			np.savetxt(wdir+'data/computed/{}/dtdm_stats/{}/{}/mjd_edges.csv'.format(self.obj, prop, self.log_or_lin), self.mjd_edges)
		print('time elapsed: {:.2f} minutes'.format((time()-start)/60.0))

	def read_pooled_stats(self, log_or_lin, key=None):
		self.log_or_lin = log_or_lin
		if key is None:
			fpath = wdir+'data/computed/{}/dtdm_stats/{}/'.format(self.obj, self.log_or_lin)
			names = listdir(fpath)
			self.pooled_stats = {name[7:-4].replace('_',' '):np.loadtxt(fpath+name) for name in names if name.startswith('pooled')}
		else:
			self.read_key(key)
			fpath = wdir+'data/computed/{}/dtdm_stats/{}/{}/'.format(self.obj, key, self.log_or_lin)
			names = listdir(fpath)
